...
public:
    ...
    std::vector<T> operator()(const std::vector<T>& in, const std::array<std::size_t, 2>& dimensions, const std::array<double, 2>& spacing) // transform by call method of class
    {
        ...
        const std::array<double, 2> forward_scaling { m_outSpacing[0] / spacing[0], m_outSpacing[1] / spacing[1] };
        ...
        const std::array<double, 2> offset { 0.5 * (dimensions[0] * spacing[0] / m_outSpacing[0] - m_outDimension[0]),
            0.5 * (dimensions[1] * spacing[1] / m_outSpacing[1] - m_outDimension[1]) };
        ...
        for (std::size_t j = 0; j < m_outDimension[1]; ++j) {
            ...
            // y-direction
            const auto y1_idx = y < 0.0 ? 0 : y > dimensions[1] - 1 ? dimensions[1] - 2 : static_cast<std::size_t>(std::floor(y));
            ...
            for (std::size_t i = 0; i < m_outDimension[0]; ++i) {
                ...
                // x-direction
                const auto x1_idx = x < 0.0 ? 0 : x > dimensions[0] - 1 ? dimensions[0] - 2 : static_cast<std::size_t>(std::floor(x));
